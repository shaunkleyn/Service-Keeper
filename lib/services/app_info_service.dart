...
class AppInfoService {
  ...
  Future<String?> getAppName(String packageName) async {
    try {
      return await _channel
          .invokeMethod<String>('getAppName', {'packageName': packageName});
    } on PlatformException {
      return null;
    }
  }
  ...
  /// Each entry has keys: p (packageName), c (serviceClass), n (appName), e (exported), pm (permission).
  Future<
      List<
          ({
            String packageName,
            ...
            bool exported,
            String permission
          })>> getInstalledServices({bool includeSystem = false}) async {
    try {
      ...
      return raw
          .map((item) {
            ...
            return (
              ...
              exported: (m['e'] as bool?) ?? false,
              permission: (m['pm'] as String?) ?? '',
            );
          })
          .where((e) => e.packageName.isNotEmpty && e.serviceClass.isNotEmpty)
          .toList();
    } on PlatformException {
      return [];
    }
